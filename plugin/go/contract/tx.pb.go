...
// ---- DCB (Data Center Builder) messages ----
// MessageDcbStartRun begins a player's season run.
type MessageDcbStartRun struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Address       []byte                 `protobuf:"bytes,1,opt,name=address,proto3" json:"address,omitempty"` // player account (also the signer)
	Name          string                 `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`       // leaderboard display name
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
// MessageDcbSetPolicy commits a new allocation policy (canonical EncodePolicy bytes).
...
// MessageDcbCheckpoint realizes idle gains by catching the player up to the head.
...
// MessageDcbBuy purchases qty units of accelerator type `kind` (0=GPU..4=MTIA).
...
// MessageDcbSell liquidates qty units of accelerator type `kind`.
...
// MessageDcbHire adds n people (multiple of 10).
...
// MessageDcbFire removes n people (multiple of 10).
...
// MessageDcbBuyInfra buys qty of a shared-infra kind (0=power,1=cooling,2=land,3=network).
...
const file_tx_proto_rawDesc = "" +
	...
	"\x06amount\x18\x03 \x01(\x04R\x06amount\"B\n" +
	...
	"\x03qty\x18\x03 \x01(\x03R\x03qty\"&\n" +
	"\tFeeParams\x12\x19\n" +
...
var file_tx_proto_goTypes = []any{
	...
	(*FeeParams)(nil),            // 10: types.FeeParams
	(*Signature)(nil),            // 11: types.Signature
	(*anypb.Any)(nil),            // 12: google.protobuf.Any
}
var file_tx_proto_depIdxs = []int32{
	12, // 0: types.Transaction.msg:type_name -> google.protobuf.Any
	11, // 1: types.Transaction.signature:type_name -> types.Signature
